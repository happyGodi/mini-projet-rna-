...
export class NeuralNetwork {
    ...
    public train(): ErrorPoint[] {
        let errorPoints: ErrorPoint[] = []
        ...
        for (const { inputs, target } of this.prototype) {
            ...
            const nmse = this.calculNMSE([output], [target]);
            errorPoints.push({ x: nmse, y: this.prototype.indexOf({ inputs, target }) });
        }
        //retourne un tableau des erreurs pour chaque prototype
        return errorPoints
    }

    //fonction de calcul de l'erreur NMSE
    private calculNMSE(predictions: number[], targets: number[]): number {
        let sumError: number = 0;
        let sumSquareTargets: number = 0;

        for (let i = 0; i < predictions.length; i++) {
            const error = parseFloat((targets[i] - predictions[i]).toPrecision(8));
            sumError += parseFloat((error ** 2).toPrecision(8));
            sumSquareTargets += parseFloat((targets[i] ** 2).toPrecision(8));
        }

        return parseFloat((sumError / sumSquareTargets).toPrecision(8));
    }
